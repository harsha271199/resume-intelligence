...
def _extract_text_from_file(uploaded_file) -> Optional[str]:
    ...
    Supports:
    - .txt  — UTF-8 decode
    - .pdf  — pypdf PdfReader (text layer only)
    ...
    if name.endswith(".pdf"):
        try:
            ...
            text = "\n".join(pages).strip()
            if not text:
                st.warning("PDF has no extractable text layer (scanned image?). Try a text-based PDF.")
            return text
...
# ── Input widgets ─────────────────────────────────────────────────────────────
#
# WHY session_state injection?
# ────────────────────────────
# st.text_area(value=...) only sets the widget's value on the *first* render.
# On every subsequent render Streamlit ignores the `value` argument and returns
# whatever is already stored in the widget's internal state key.
#
# This means that when a file is uploaded (which triggers a rerun), the text
# area still holds its previous value — usually empty — so `resume_text` stays
# blank and the validation check fires "Please provide resume text".
#
# The correct fix is to write the extracted text directly into
# st.session_state[<widget_key>] *before* the widget is rendered.
# Streamlit reads session_state first, so the widget will display the new text
# on the very same rerun that the file was uploaded.
...
def _resolve_text(
    ...
    """
    Determine the active text for one input slot and inject it into
    session_state so the text area widget reflects it immediately.

    Priority:
    1. Uploaded file  (PDF / DOCX / TXT)
    2. Use Sample Data toggle
    3. Whatever the user typed in the text area (preserved as-is)

    Returns the resolved text string (may be empty if none of the above apply).
    """
    uploaded = st.session_state.get(file_uploader_key)

    if uploaded is not None:
        # Track which file we last processed to avoid re-extracting on every rerun
        ...
        if st.session_state.get(last_id_key) != file_id:
            ...
            if extracted and extracted.strip():
                # ← KEY FIX: write into the text area's session_state key
                st.session_state[text_area_key] = extracted
                st.session_state[last_id_key] = file_id
            else:
                st.session_state[last_id_key] = file_id  # mark as processed even on failure

    elif use_sample:
        # Only inject sample data if the text area is currently empty
        if not st.session_state.get(text_area_key, "").strip():
            st.session_state[text_area_key] = sample_text

    # Return whatever is now in the text area's state (file text, sample, or typed)
    return st.session_state.get(text_area_key, "")


def _render_inputs(use_sample: bool) -> tuple[str, str]:
    """
    Render resume and job description input columns.

    Priority order for each input:
    1. Uploaded file (PDF / DOCX / TXT)  ← always wins
    2. Use Sample Data sidebar toggle
    3. Text typed directly into the text area
    """
    ...
    with col1:
        ...
        # File uploader — stores result in st.session_state["resume_file"]
        st.file_uploader(
        ...
        # Resolve text and inject into session_state BEFORE the text area renders
        _resolve_text(
            file_uploader_key="resume_file",
            text_area_key="resume_text",
            use_sample=use_sample,
            sample_text=_SAMPLE_RESUME,
            label="resume",
        )
        ...
        resume_text = st.text_area(
            ...
            key="resume_text",  # value comes from session_state injection above
        )
    ...
    with col2:
        ...
        _resolve_text(
            file_uploader_key="jd_file",
            text_area_key="jd_text",
            use_sample=use_sample,
            sample_text=_SAMPLE_JD,
...
def _render_pipeline_status(agents_completed: list[str]) -> None:
    """Show which agents completed with tick/cross indicators."""
    agents = [
...
def _render_scoring_section(result, show_tfidf: bool, show_raw: bool) -> None:
    ...
    c1.metric("Semantic Similarity", f"{sr.semantic_similarity:.1f}",
              help="MiniLM cosine similarity (0–100)")
    c2.metric("Skill Coverage", f"{sr.skill_coverage:.1f}",
              help="% of required JD skills present in resume")
    c3.metric("Experience Match", f"{sr.experience_match:.1f}",
              help="Candidate years vs. required years")
    c4.metric("Skill Gap Penalty", f"{sr.skill_gap_penalty:.1f}",
              help="Higher = fewer gaps (better)")

    if show_tfidf and sr.tfidf_baseline_score is not None:
        st.caption(
            f"TF-IDF keyword baseline: **{sr.tfidf_baseline_score:.1f}** / 100  "
            f"(neural model adds {sr.final_score - sr.tfidf_baseline_score:+.1f} pts)"
        )
    ...
    with col1:
        ...
        if sr.semantic_matches:
            with st.expander(f"🔍 Semantic Matches ({len(sr.semantic_matches)})"):
                st.caption("Semantically related to your resume content but not an exact match.")
...
def _render_audit_section(result, show_raw: bool) -> None:
    ...
    c1.metric("Baseline Score", f"{fr.original_score:.1f}",
              help="Score for the original resume name")
    c2.metric("Max Score Shift", f"{fr.max_difference:.2f} pts",
              help="Largest absolute score change across all name variants")
    c3.metric("Demographic Parity Diff", f"{fr.demographic_parity_difference:.4f}",
              help="Max group avg − min group avg across demographic groups")

    # Colour-coded summary banner
...
def main() -> None:
    ...
    # Cache key: only re-run when inputs or audit toggle change.
    # Read from session_state so file-uploaded text is included even before
    # the text area widget has been interacted with.
    _resume_for_cache = st.session_state.get("resume_text", resume_text)
    _jd_for_cache = st.session_state.get("jd_text", jd_text)
    cache_key = (_resume_for_cache.strip(), _jd_for_cache.strip(), options["run_audit"])

    if analyze:
        # Use the session_state values directly — these are always up-to-date
        # regardless of whether text came from typing, file upload, or sample data.
        resume_text = st.session_state.get("resume_text", resume_text)
        jd_text = st.session_state.get("jd_text", jd_text)
    ...
    if "_result" in st.session_state:
        ...
        # No results yet — show landing page
        _render_landing()
